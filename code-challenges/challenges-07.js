...
const applicationsStatics = (arr) => {
    // write your code here
    let javaScriptDevs=0 ,pythonDevs=0,dotNetDevs=0,javaDevs=0,total=0,rejected=0;
    for(let i=0;i<=arr.length-1;i++){
        (arr[i].tech.toLowerCase==="JS")?javaScriptDevs++:
        (arr[i].tech==="Python")?pythonDevs++:
        (arr[i].tech===".Net")?dotNetDevs++:
        javaDevs++;
        ((arr[i].firstName==null&&arr[i].lastName==null)||(arr[i].firstName==""&&arr[i].lastName=="")||(arr[i].yearsOfExperience<1))?rejected++:
...
const classesAvg = (data) => {
    ...
    for(let i=0;i<=data.grades.length-1;i++){
        for(let j=0;j<=data.grades[i].classes.length-1;j++){
            for(let k=0;k<=data.grades[i].classes[j].classScores.length-1;k++){
